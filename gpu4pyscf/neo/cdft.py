...
from gpu4pyscf.lib.cupy_helper import to_cupy
...
def solve_constraint(mf, fock0, s1e=None, f_lagrange_guess=None):
    ...
    # opt = scipy.optimize.root(position_deviation, f_lagrange_guess.get(), method='hybr')
...
class CDFT(ks.KS):
        ...
        for t, comp in self.components.items():
            if t.startswith('n'):
                ...
                comp.int1e_r = to_cupy(comp.mol.intor_symmetric('int1e_r', comp=3)) \
                             - cupy.asarray([comp.nuclear_expect_position[i] * s1e for i in range(3)])
                comp.int1e_r_symm = None
                if comp.mol.symmetry and comp.mol._symm_axes is not None:
                    # Transform to along symmetry axes
                    comp.int1e_r_symm = cupy.einsum('xy,yij->xij', comp.mol._symm_axes, comp.int1e_r)
    ...
    def get_fock_add_cdft(self):
        ...
        for t, comp in self.components.items():
            if t.startswith('n'):
                ...
                f_add[t] = cupy.einsum('xij,x->ij', comp.int1e_r, self.f[ia])
        return f_add
    ...
    def dip_moment(self, mol=None, dm=None, unit='Debye', origin=None,
        ...
        if origin is None:
            ...
            origin = cupy.asarray(origin, dtype=cupy.float64)
