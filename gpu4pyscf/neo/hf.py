...
def dot_eri_dm(eri, dms, nao_v=None, eri_dot_dm=True):
    ...
    for i in range(n_dm):
        pyscf_lib.hermi_triu(vj[i], 1, inplace=True)
    if len(dms_shape) == 2:
...
def init_guess_mixed(mol, mixing_parameter = cupy.pi/4):
    ...
    h1e = to_cupy(scf.hf.get_hcore(mol))
    s1e = to_cupy(scf.hf.get_ovlp(mol))
...
class ComponentSCF(Component):
    ...
    def get_occ(self, mo_energy=None, mo_coeff=None):
        ...
        if self.is_nucleus:
            ...
            if self.mol.symmetry:
                ...
                rest_idx = cupy.ones(mo_occ.size, dtype=bool)
                nelec_fix = 0
                for i, ir in enumerate(mol.irrep_id):
                    irname = mol.irrep_name[i]
                    if irname in self.irrep_nelec:
                        ir_idx = cupy.where(orbsym == ir)[0]
                        n = self.irrep_nelec[irname]
                        occ_sort = cupy.argsort(mo_energy[ir_idx].round(9), kind='stable')
                ...
                if nelec_float > 0:
                    rest_idx = cupy.where(rest_idx)[0]
                    occ_sort = cupy.argsort(mo_energy[rest_idx].round(9), kind='stable')
                    occ_idx  = rest_idx[occ_sort[:nelec_float//2]]
                    mo_occ[occ_idx] = self.mol.nnuc

                assert cupy.sum(mo_occ > 0) == 1 # ensure singly occupied

                vir_idx = (mo_occ==0)
                if self.verbose >= logger.INFO and cupy.count_nonzero(vir_idx) > 0:
    ...
    def to_gpu(self):
        ...
        return pyscf_lib.to_gpu(self, obj)
...
class InteractionCoulomb:
    ...
    def _is_mem_enough(self):
        ...
        return nao1**2*nao2**2*2/1e6+pyscf_lib.current_memory()[0] < self.max_memory*.95
    ...
    def get_vint(self, dm):
        ...
        if (not mol.direct_vee and
            ...
            if dm2 is not None:
                vj[self.mf1_type] = dot_eri_dm(self._eri, dm2.get(),
                                               nao_v=mol1.nao, eri_dot_dm=True)
            if dm1 is not None:
                vj[self.mf2_type] = dot_eri_dm(self._eri, dm1.get(),
        ...
        if self.mf1_type in vj:
            ...
            vj[self.mf1_type] = to_cupy(vj[self.mf1_type])
        if self.mf2_type in vj:
            vj[self.mf2_type] *= charge_product
            vj[self.mf2_type] = to_cupy(vj[self.mf2_type])
        return vj
        ...
        if self.mol.symmetry:
            for t, comp in self.components.items():
                if isinstance(comp, scf.uhf.UHF):
                    ea = cupy.hstack(comp.mo_energy[0])
                    eb = cupy.hstack(comp.mo_energy[1])
                    ...
                    oa_sort = cupy.argsort(ea[comp.mo_occ[0]>0 ].round(9), kind='stable')
                    va_sort = cupy.argsort(ea[comp.mo_occ[0]==0].round(9), kind='stable')
                    ob_sort = cupy.argsort(eb[comp.mo_occ[1]>0 ].round(9), kind='stable')
                    vb_sort = cupy.argsort(eb[comp.mo_occ[1]==0].round(9), kind='stable')
                    idxa = cupy.arange(ea.size)
                    idxa = cupy.hstack((idxa[comp.mo_occ[0]> 0][oa_sort],
                                         idxa[comp.mo_occ[0]==0][va_sort]))
                    idxb = cupy.arange(eb.size)
                    idxb = cupy.hstack((idxb[comp.mo_occ[1]> 0][ob_sort],
                    ...
                    # Using mergesort because it is stable. We don't want to change the
                    # ordering of the symmetry labels when two orbitals are degenerated.
                    o_sort = cupy.argsort(comp.mo_energy[comp.mo_occ> 0].round(9), kind='stable')
                    v_sort = cupy.argsort(comp.mo_energy[comp.mo_occ==0].round(9), kind='stable')
                    idx = cupy.arange(comp.mo_energy.size)
                    idx = cupy.hstack((idx[comp.mo_occ> 0][o_sort],
